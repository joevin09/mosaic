    ...
    protected function _compile_queries() {
        ...
        foreach ($dbs as $db) {
            if (count($db->queries) == 0) {
                $output[] = array($db->database => $this->CI->lang->line('profiler_no_queries'));
            } else {
                ...
                foreach ($db->queries as $key => $val) {
                    ...
                    $explain = strpos($val, 'SELECT') !== false ? $db->query('EXPLAIN ' . $val) : NULL;
                    if (!is_null($explain)) {
...
//die('<pre>' .print_r($output, true));
    ...
    /**
     * Compile my_debug Data
     *
     * @return	string
     */
    protected function _compile_my_debug() {
        $my_debug = $GLOBALS['ouput_in_controller'];
        $output = "\n\n";
        $output .= '<fieldset id="ci_profiler_controller_debug" style="border:1px solid #0fa7ff;padding:6px 10px 10px 10px;margin:20px 0 20px 0;background-color:#eee">';
        $output .= "\n";
        $output .= '<legend style="color:#0fa7ff;">&nbsp;&nbsp;' . $this->CI->lang->line('profiler_controller_debug') . '&nbsp;&nbsp;';
        if (!empty($this->CI->controller_debug)) {
            $output .= '(<span style="cursor: pointer;" onclick="var s=document.getElementById(\'profiler_controller_debug\').style;s.display=s.display==\'none\'?\'\':\'none\';this.innerHTML=this.innerHTML==\'' . $this->CI->lang->line('profiler_section_show') . '\'?\'' . $this->CI->lang->line('profiler_section_hide') . '\':\'' . $this->CI->lang->line('profiler_section_show') . '\';">' . $this->CI->lang->line('profiler_section_show') . '</span>)';
        }
        $output .= "</legend>\n";
        if (empty($this->CI->controller_debug)) {
            $output .= "<div style='color:#0fa7ff;font-weight:normal;padding:4px 0 4px 0'>" . $this->CI->lang->line('profiler_no_controller_debug') . "</div>";
        } else {
            $output .= "\n\n<table style='width:100%; border:none; display:none;' id='profiler_controller_debug'>\n";
            $output .= "<tr><td style='width:100%;color:#000;background-color:#ddd;padding:5px'>" . $this->CI->controller_debug . "</td></tr>\n";
            $output .= "</table>\n";
        }
        $output .= "</fieldset>";
